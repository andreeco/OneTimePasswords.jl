...
!!! warning
    ...
    It is **stateless** — it does not include rate limiting, lockout, replay tracking, throttling, or secure memory handling.
    ...
    Secrets are emitted as Base32‑encoded immutable `String`s.  
    They cannot be zeroized.  
    For stricter security, store secrets as `Vector{UInt8}` and overwrite them with `fill!()`.
